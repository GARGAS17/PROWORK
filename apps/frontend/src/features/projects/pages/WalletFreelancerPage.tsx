...
import { Wallet, Loader2, ArrowDownRight, Clock, CheckCircle2 } from 'lucide-react';
...
export const WalletFreelancerPage = () => {
  const { token } = useAuth();
  ...
  const totalEarnings = payments.filter(p => p.status === 'released').reduce((acc, curr) => acc + Number(curr.freelancer_amount || curr.amount), 0);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl mx-auto p-4 md:p-8">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-2">Gestiona tus ganancias y pagos retenidos en garantía (Escrow).</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-3xl p-8 text-white shadow-xl shadow-indigo-500/20 relative overflow-hidden">
          <div className="relative z-10">
            <p className="text-indigo-100 font-bold uppercase tracking-wider text-sm mb-2">Total Ganancias Disponibles</p>
            <h2 className="text-5xl font-black">${totalEarnings.toFixed(2)}</h2>
            <p className="mt-4 text-sm text-indigo-200">Dinero liberado y listo para retirar.</p>
          </div>
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-8 shadow-sm">
          <p className="text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider text-sm mb-2">Pagos en Garantía (Escrow)</p>
          <h2 className="text-5xl font-black text-gray-900 dark:text-white">${totalEscrow.toFixed(2)}</h2>
          <p className="mt-4 text-sm text-gray-500 dark:text-gray-400 flex items-center gap-2">
            <Clock className="w-4 h-4" /> Dinero seguro esperando la liberación de la empresa.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl overflow-hidden shadow-sm">
        ...
        {isLoading ? (
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400">Tus pagos aparecerán aquí cuando seas contratado.</p>
          </div>
        ) : (
              ...
              <div key={payment.id} className="p-6 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                ...
                <div className="text-right">
                  <p className="text-xl font-black text-gray-900 dark:text-white">
                    +${payment.status === 'released' ? Number(payment.freelancer_amount || payment.amount).toFixed(2) : Number(payment.amount).toFixed(2)}
                  </p>
                  {payment.status === 'released' ? (
